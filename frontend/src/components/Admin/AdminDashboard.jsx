...
import { FiSearch, FiChevronDown, FiChevronUp, FiGrid, FiList, FiMenu, FiEdit, 
         FiTrash2, FiPlus, FiInfo, FiFolder, FiFolderPlus } from 'react-icons/fi';
...
const GptCard = React.memo(({ gpt, onDelete, onEdit, formatDate, onNavigate, isDarkMode, onMoveToFolder }) => (
    <div 
        key={gpt._id} 
...
const AdminDashboard = ({ userName = "Admin User" }) => {
    ...
    const [folders, setFolders] = useState(['All', 'Uncategorized']);
    ...
    // Handle create new GPT
    const handleCreateNew = () => {
        setShowCreateGpt(true);
    };
    ...
    const handleMoveToFolder = (gpt) => {
        setGptToMove(gpt);
        setShowMoveModal(true);
    };

    // Handle successful move
    const handleGptMoved = (movedGpt, newFolderName) => {
    ...
    return (
        <div className="flex h-screen bg-white dark:bg-black text-black dark:text-white overflow-hidden">
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                {!showCreateGpt ? (
                    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900 text-black dark:text-white">
                        {/* Header */}
                        ...
                        {/* Controls: Folder, Sort */}
                        <div className="flex flex-col md:flex-row md:items-center justify-between px-4 sm:px-6 py-3 gap-3 md:gap-4 flex-shrink-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
